...
const NotificacionesPage = () => {
  ...
  const fetchNotificaciones = async () => {
    try {
      ...
      const response = await axiosAuth.get('/api/hojas-ruta/dashboard/tiempo-real');
  ...
  const marcarComoLeida = async (notifId: number) => {
    try {
      await axiosAuth.patch(`/api/notificaciones/${notifId}/leer`, {});
  ...
  const marcarTodasLeidas = async () => {
    try {
      await axiosAuth.patch('/api/notificaciones/marcar-todas-leidas', {});
